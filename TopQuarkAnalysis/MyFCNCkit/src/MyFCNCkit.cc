...
// 	  const reco::Candidate* dau2 = NULL;
   ...
   for(std::vector<pat::MET>::const_iterator it_pfmet =METHandle->begin();
       it_pfmet != METHandle->end(); it_pfmet++ ){
     
     EvtInfo.MET           = it_pfmet->pt();
     EvtInfo.METPhi        = it_pfmet->phi();
     EvtInfo.RawMET        = it_pfmet->uncorrectedPt();
     EvtInfo.RawMETPhi     = it_pfmet->uncorrectedPhi();
     
     EvtInfo.METx          = it_pfmet->px(); //Uly 2011-04-04
     EvtInfo.METy          = it_pfmet->py(); //Uly 2011-04-04
     
     EvtInfo.SumEt           = it_pfmet->sumEt();
     EvtInfo.METSig        = it_pfmet->mEtSig();
