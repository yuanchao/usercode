...
void AnalysisRootpleProducer::beginJob( const EventSetup& )
{
  ...
  AnalysisTree->Branch("EventScale",&EventScale,"EventScale/D");
  
  AnalysisTree->Branch("FilterEff",&FilterEff,"FilterEff/D");
  
  AnalysisTree->Branch("CrossSect",&CrossSect,"CrossSect/D");
...
void AnalysisRootpleProducer::analyze( const Event& e, const EventSetup& )
{
  ...
  // Retreve event info from AODSIM
  Handle< int > genProcessID;
  e.getByLabel( mcEvent.c_str(), genProcessID ) ;
  EventKind = *genProcessID;
  //cout << "genProcessID " << EventKind << endl;

  Handle< double > genEventScale;
  e.getByLabel( "genEventScale", genEventScale );
  EventScale = *genEventScale;
  
  Handle< double > genFilterEff;
  e.getByLabel( "genEventRunInfo", "FilterEfficiency", genFilterEff);
  FilterEff = *genFilterEff;
 
  Handle< double > genCrossSect;
  e.getByLabel( "genEventRunInfo", "PreCalculatedCrossSection", genCrossSect); 
  CrossSect = *genCrossSect;
