...
// Wed Jan 14 14:56:16 2015 by ROOT version 5.34/07
...
#ifdef MyCZp_cxx
MyCZp::MyCZp(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("zprime.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("zprime.root");
      }
...
Int_t MyCZp::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MyCZp::LoadTree(Long64_t entry)
{
...
void MyCZp::Show(Long64_t entry)
{
...
Int_t MyCZp::Cut(Long64_t entry)
{
